...
arr=(`echo $text`)
ssh ${USER}@${HOST} "docker exec -i ${arr[0]} /usr/bin/mysqldump -u root -p\"${MSPASSWD}\" -x --all-databases > dump.sql"
...
ssh ${USER}@${HOST} "docker exec -i ${arr[0]} /usr/bin/mysqladmin -u root password \"${MSPASSWD}\""
ssh ${USER}@${HOST} "docker exec -i ${arr[0]} mysql -u root -p\"${MSPASSWD}\" < /data_mysql/dump.sql"
